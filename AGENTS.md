...
- `.github/workflows/ci.yml` is the authoritative clean-run check when it runs. It installs locked dependencies, runs the complete Vitest suite, checks JavaScript syntax, and runs `wrangler deploy --dry-run` without deploying.
