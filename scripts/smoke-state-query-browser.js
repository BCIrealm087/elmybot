// Chromium UI smoke against a deterministic HTTP/SSE contract fixture.
...
function send(stream, event = "update", reason = "value_change") {
  stream.sequence += 1;
  const results = stream.queries.map(({ id, query }) => ({ queryId: id, sequence: stream.sequence, status: "ready", reason, result: evaluate(query) }));
  stream.response.write(`id: fixture-${stream.sequence}\nevent: ${event}\ndata: ${JSON.stringify({ protocol: "state-query-stream/v1", results })}\n\n`);
}
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/state-query/stream") {
      ...
      const stream = { response, queries: JSON.parse(body).queries, sequence: 0 };
      streams.add(stream); response.on("close", () => streams.delete(stream)); send(stream, "snapshot", "initial"); return;
    }
...
try {
  ...
  await widget.waitForFunction(() => document.getElementById("connection").textContent === "Live");
  for (const stream of streams) stream.response.write(`event: status\ndata: ${JSON.stringify({ protocol: "state-query-stream/v1", results: stream.queries.map(({ id }) => ({ queryId: id, status: "denied", error: { code: "query_grant_revoked" } })) })}\n\n`);
  ...
  console.log("Chromium smoke passed: custom composition, isolated session, widget count/game/source updates, unselected state, text safety, reconnect, revocation, and teardown.");
} finally {
  await browser?.close();
  for (const stream of streams) stream.response.end();
